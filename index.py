...
# Optional: Pywidevine imports (will be used only in DRM path)
try:
...
def get_cached_token_internal():
    ...
    user_id_str = request.args.get('user_id') or "unknown"
...
# === TELEGRAM & LOGGING ===
def log_request(user_id, url, token_used=None):
    log_entry = {
    ...
    if len(recent_logs) > MAX_RECENT_LOGS:
        recent_logs.pop(0)
    if TELEGRAM_BOT_TOKEN and YOUR_TELEGRAM_CHAT_ID:
        try:
            msg = f"New Request\nUser ID: {user_id}\nURL: {url[:80]}..."
            requests.post(
                f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
                data={"chat_id": YOUR_TELEGRAM_CHAT_ID, "text": msg},
                timeout=10
            )
        except Exception as e:
...
def ITsGOLU_OFFICIAL():
    ...
    if "error" not in result:
        log_request(user_id, actual_url, token)
